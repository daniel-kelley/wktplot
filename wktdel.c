...
static int w_delaunay(struct info *info)
{
    info->geom = GEOSDelaunayTriangulation_r(
        info->wkt.handle,
        info->wkt.geom,
        info->tolerance,
        info->only_edges);
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "t:Bbevh")) != EOF) {
        switch (c) {
        case 't':
            info.tolerance = strtod(optarg,0);
            break;
        case 'e':
            info.only_edges = 1;
            break;
